...
void main() {
	...
	FragColor.rgb *= diff * ColorModifier;
	...
	float sceneLerp = gl_FragCoord.z > nonLinearDepth ? 1.0 : 0.0;

	vec4 SceneColor = texture(_SceneColor0, vec2(ndc.s, ndc.t));

	float alpha = texture(_AlphaTexture, TexCoords).r;

	FragColor.rgb = mix(FragColor.rgb, SceneColor.rgb, sceneLerp);

	/*if(texture(_AlphaTexture, TexCoords).r < 0.25) {
		//discard;
	}*/

	

	float fade = smoothstep(0.0, 0.2, TexCoords.t);
	float noiseValue = random(TexCoords);
	fade = mix(fade, fade * noiseValue, 1.0 - smoothstep(0.1, 0.3, TexCoords.t));

	FragColor.a = DistanceFadeValue * alpha;
}