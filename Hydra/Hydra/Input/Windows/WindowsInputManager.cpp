...
void WindowsInputManager::Update(SharedPtr<UIWindow> window)
{
	static bool isFirstCapture = false;
	...
	if (_MouseCaptured)
	{
		...
		int centerX = static_cast<int>(rect.left + (rect.right - rect.left) / 2.0f);
		int centerY = static_cast<int>(rect.top + (rect.bottom - rect.top) / 2.0f);
		...
		if (isFirstCapture)
		{
			_MousePos.x = _LastMousePos.x;
			_MousePos.y = _LastMousePos.y;
		}
