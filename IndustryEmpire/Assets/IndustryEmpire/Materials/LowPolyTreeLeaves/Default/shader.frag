...
void main() {
	...
	vec3 objectColor = _Color;

#ifdef USE_TEXTURE
	vec4 fullObjectColor = texture(_Texture, TexCoords);
	objectColor = fullObjectColor.rgb;

#ifdef USE_ALPHA
	if(fullObjectColor.a < 0.5) discard;
#endif
	...
	vec3 result = (AmbientLight.rgb + diffuse) * (objectColor + albedoColor.rgb) * ColorModifier;
