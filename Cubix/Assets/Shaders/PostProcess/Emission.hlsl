...
Texture2D _SceneTexture : register(t0);
Texture2D _EmissionTexture : register(t1);

float4 MainPS(FullScreenQuadOutput IN) : SV_Target
{
	float3 sceneColor = _SceneTexture[IN.position.xy].rgb;

	sceneColor += _EmissionTexture[IN.position.xy].rgb;
